...
def gripper_move_to_object(x1,y1,Degree):
    ...
    group1.execute(plan,wait=True) 
    # rospy.sleep(0.5)
    # rospy.sleep(1) # Finish Pick

    # waypoints = []
    # print(waypoints)
    # robot_pose.position.z = 0.82
    # waypoints.append(copy.deepcopy(robot_pose))
    # robot_pose.position.z = 1.3
    # waypoints.append(copy.deepcopy(robot_pose))
    # robot_pose.position.x = x2
    # robot_pose.position.y = y2
    # waypoints.append(copy.deepcopy(robot_pose))
    # plan, fraction = group1.compute_cartesian_path(waypoints,0.1,0.0) 
    # group1.execute(plan,wait=True)
    # rospy.sleep(0.5)
...
# group2.set_num_planning_attempts(10)
# group2.set_planning_time(10.0)
# group2.set_max_velocity_scaling_factor(1.0)
...
# group1.set_max_velocity_scaling_factor(1.0)
# group1.set_max_acceleration_scaling_factor(1.0)
...
for count in range(3):
    for num in range(15):
        parts_pose = part_pose_collect()
        if (abs(parts_pose[num][0])>30.0 and  abs(parts_pose[num][1])>30.0):
            continue

        if(num >= 0 and num <5):
            basket_pose=[-0.55,0.60,1.3]
            direc = parts_pose[num][5]
        elif(num >= 5 and num <10):
            basket_pose=[-0.55,0.40,1.3]
            direc = parts_pose[num][5]
        else:
            basket_pose=[-0.55,-0.60,1.3]
            direc = parts_pose[num][4]

        # robot_move_line(parts_pose[num][0], parts_pose[num][1], 1.3, basket_pose[0], basket_pose[1], 1.3)
        # gripper_move_to_object(parts_pose[num][0], parts_pose[num][1],direc)
        
        goal_move_line(parts_pose[num][0],parts_pose[num][1],1.3)
        robot_move_line(parts_pose[num][0], parts_pose[num][1], 1.3, parts_pose[num][0], parts_pose[num][1], 0.88)
        gripper_control(True)
        rospy.sleep(1)
        robot_move_line(parts_pose[num][0], parts_pose[num][1], 0.88, parts_pose[num][0], parts_pose[num][1], 1.3)
        robot_move_line(parts_pose[num][0], parts_pose[num][1], 1.3, basket_pose[0], basket_pose[1], 1.3)
        gripper_control(False)
        rospy.sleep(1)
